...
import { deleteProjectFromList, setprojectList } from "../features/projectSlice";
...
function DeleteProject() {
  ...
  useEffect(() => {
    if (data) {   
      dispatch(deleteProjectFromList(data.id));
      }
  }, [data]);
  if (isLoading) {
        ...
        <ul className=" px-4 overflow-y-scroll max-h-96 mt-2 border-t-2 border-indigo-500">
          {projectList &&
            projectList.map((item) => {
              return (
                <li  key = {item.id} className ={`py-4 px-2 border-b-2 border-indigo-200 ${item && item.is_completed && "bg-green-400"}`} >
